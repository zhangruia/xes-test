  ...
  if (current.name == 'stem') {
    ...
    if (parent && parent.conName == 'Choice') {
      if (prev) {
        ...
        isWarp(prev, current, parent)
      } else {
        isWarp(null, current, parent)
      }
    }
  }
}
...
function getLedalContent (iswarp, prev, current, parent){
  ...
  if (iswarp) {
    ...
    if (prev && !forceWarp) totalWid = parent.rectangle[2] - prev.rectangle[2]
    else totalWid = parent.rectangle[2]
  }

  for (let i of text) {
    // console.log(totalWid, wid);
    if (totalWid - wid >= 30) {
      /*
        *******
        * 该判断条件成立下再同一行
  ...
  // console.log('可容纳：' + accomm);
  // console.log('剩余：' + residue);
  // console.log(residue);
  return {
    accomm: accomm,
    residue: residue
...
function countWidth (iswarp, prev, text, parent) {
  ...
  if (iswarp) {
    ...
    if (parent) {
      totalWid = parent.rectangle[2]
    }
  } else {
    if (prev && !forceWarp) {
      totalWid = parent.rectangle[2] - prev.rectangle[2]
    } else {
      totalWid = parent.rectangle[2]
    }
  }
